...
public class XLResponse {
    private byte encode;// 数据编码格式。已定义：0：UTF-8，1：GBK，2：GB，3：ISO-1
    private byte encrypt;// 加密类型。0表示不加密
    private byte command;// 用于扩展协议。暂未定义任何值
    private byte extend2;// 用于扩展协议。暂未定义任何值
    private int sessionid = 0;// SeqID
    private int result = 0;// 结果码
    private int length = 0;// 数据包长
...
*/
    ...
    public String toString() {
        return "XLResponse [encode=" + encode + ", encrypt=" + encrypt + ", command=" + command + ", extend2=" + extend2
                + ", sessionid=" + sessionid + ", result=" + result + ", length=" + length + ", values=" + values + "]";
    }
}
